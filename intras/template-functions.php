        ...
        if( $facilitadores ){
            ...
            if( isset( $term_sipgo_id[0] ) ){
                ...
                if( $results && $results != '' | $results != null ){
                    ...
                    $intras_sites = json_decode( $results );

                    // Si no viene nada
                    if( $intras_sites == null ){
                        return false;
                    }

                    if( count( $intras_sites ) > 1 ){

                        return $intras_sites;

                    }elseif( str_contains( $intras_sites[0]->link, check_site_exists($dominio_actual, $sites) ) ){
